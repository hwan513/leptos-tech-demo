...
#[component]
pub fn ShoppingCart(#[prop(into)] title: String) -> impl IntoView {
        ...
        <div class=style::cart>
            ...
            <div class=style::cart_container>
                {items_clone
                    ...
                    .map(|(index, item)| {
                        view! {
                            <ShoppingCartItem count=move || groups()[index] item=item.clone()/>
                        }
                    })
                    .collect_view()}
            </div>
...
#[component]
fn ShoppingCartItem<F>(count: F, item: Item) -> impl IntoView
where
    F: Fn() -> usize + 'static,
{
